...
export default function Results() {
  ...
  // Fetch summary + send email on mount
  useEffect(() => {
  ...
  // When summaryData is committed to state, derive letter categories.
  // Using a separate useEffect guarantees we read the *new* summaryData value,
  // not a stale closure from the fetch function.
  useEffect(() => {
  ...
  const getLetterColor = (letter: string) => {
    ...
    if (perf.accuracy >= 60) return "bg-yellow-400 text-white";
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-300 via-pink-300 to-yellow-200 flex items-center justify-center p-6">
      <div className="w-full max-w-7xl bg-white rounded-[3rem] shadow-2xl p-10 space-y-8">
        ...
        <div className="text-center">
          ...
          <h1 className="text-5xl font-extrabold text-purple-600 mb-2">Quiz Complete!</h1>
          <p className="text-xl text-gray-600">Great job completing the handwriting quiz! 🎉</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-purple-100 to-purple-200 rounded-3xl p-8 text-center shadow-lg">
            <div className="text-6xl font-extrabold text-purple-600 mb-2">
              {score}<span className="text-3xl text-purple-400">/{total}</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-green-100 to-green-200 rounded-3xl p-8 text-center shadow-lg">
            <div className="text-6xl font-extrabold text-green-600 mb-2">{accuracy}%</div>
            ...
            <div className="flex flex-wrap gap-3">
              {practiceLetters.length > 0 ? (
                practiceLetters.map((l) => {
                  const isRed = letterPerformance[l]?.accuracy < 60;
                  return (
                    <span
                      key={l}
                      title={getLetterTooltip(l)}
                      className={`w-14 h-14 flex items-center justify-center rounded-2xl text-white text-2xl font-extrabold shadow-md ${isRed ? "bg-red-400" : "bg-yellow-400"
                        }`}
                    >
                      {l}
                    </span>
                ...
                <p className="text-gray-500">
                  {summaryData ? "All perfect! 🌟" : "Loading..."}
                </p>
              )}
            </div>
          </div>
        ...
        >
          ...
          {emailStatus === "sent" && (
            <div className="text-center">
              ...
              <p className="text-gray-600">
                A detailed handwriting analysis report with AI-generated insights and practice
                suggestions has been sent to <strong>{parentEmail}</strong>
              </p>
